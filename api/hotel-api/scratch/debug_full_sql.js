...
async function debugQuery() {
    ...
    try {
        ...
        if (res.rows.length > 0) {
            ...
            console.log("Checking raw dat_phong for 'Đã đặt cọc'...");
            const raw = await pool.query("SELECT ma_dat_phong, trang_thai FROM public.dat_phong WHERE trang_thai = 'Đã đặt cọc'");
            console.log("Raw count:", raw.rows.length);
            console.log("Raw items:", raw.rows);
        }
    } catch (err) {
